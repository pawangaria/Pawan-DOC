...
public class LoginLogOutTest_TC2 extends LoginLogOutSuiteBase{
	...
	String runmodes[]=null;
		...
		public void checkTestSkip()
		{
				...
				//Instead of passing TestCase name,using this "this.getClass().getSimpleName()" to get the name of the class as the name of the class is the TestCase name.
			{
				...
				APP_LOGS.debug("Skipping test case LoginLogOutTest_TC2(Login With FACEBOOK ) as the runmode is NO");
				throw new SkipException("Skipping this testcase LoginLogOutTest_TC2(Login With FACEBOOK ) as the runmode is NO for this testcase");
		...
		public void LogInLogOutCheck() throws InterruptedException
		{
			...
			APP_LOGS.debug("****** Starting the Test for LoginLogOutTest_TC2(Login With FACEBOOK Account *******");
			...
			//User Log in from the LoginUser Method.
			  Loginlogout.loginByFacebook(CONFIG.getProperty("FacebookTESTUSER"),CONFIG.getProperty("FacebookTESTUSER_PASSWORD"));
			  
			  APP_LOGS.debug("User Logged in Successfully with Facebook Account now Loggin Out");
				
			  //User Log out from the Log out method.
			  if(Loginlogout.checkuserloginStatus())
			  {
			  Loginlogout.logoutFacebook();
			  }
			  APP_LOGS.debug("****** Completed the Test for LoginLogOutTest_TC2(Login With FACEBOOK) *******");
			
			}catch(Throwable t)
			{
				APP_LOGS.debug("******Execution of the LoginLogOutTest_TC2(Login With FACEBOOK) is Falied******");
