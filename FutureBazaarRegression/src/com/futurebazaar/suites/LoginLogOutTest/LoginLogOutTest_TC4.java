...
public class LoginLogOutTest_TC4 extends LoginLogOutSuiteBase{
	...
	String runmodes[]=null;
		...
		public void checkTestSkip()
		{
				...
				//Instead of passing TestCase name,using this "this.getClass().getSimpleName()" to get the name of the class as the name of the class is the TestCase name.
			{
				...
				APP_LOGS.debug("Skipping test case LoginLogOutTest_TC4(Forgot Password Link Check) as the runmode is NO");
				throw new SkipException("Skipping this testcase LoginLogOutTest_TC4(Forgot Password Check) as the runmode is NO for this testcase");
		...
		@Test
		public void ForgotPasswordLinkCheck() throws InterruptedException
		{
			...
			APP_LOGS.debug("****** Starting the Test for LoginLogOutTest_TC4(Forgot Password Link Check)*******");
			...
			//User Log in from the LoginUser Method.
			  Loginlogout.forgotPassword(CONFIG.getProperty("testuser"));
			  
			  APP_LOGS.debug("Forgot Password Link Checked from the My Account Login");
			  APP_LOGS.debug("Opening the home page by clicking the Futurebazaar logo on home Page");
				...
				APP_LOGS.debug("Checking Forgot Password Link Login Page");
			  Loginlogout.forgotPasswordByMyAccount(CONFIG.getProperty("testuser"));
			  APP_LOGS.debug("Forgot Password Link Checked from the My Account-> Login page");

			  
			  APP_LOGS.debug("****** Completed the Test for LoginLogOutTest_TC4(Forgot Password Link Check)*******");
			
			}catch(Throwable t)
			{
				APP_LOGS.debug("******Execution of the LoginLogOutTest_TC4(Forgot Password Link Check) is falied******");
