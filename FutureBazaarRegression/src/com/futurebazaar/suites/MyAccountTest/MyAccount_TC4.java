...
public class MyAccount_TC4 extends MyAccountSuitebase{
	...
	String runmodes[]=null;
		...
		public void checkTestSkip()
		{
				...
				//Instead of passing TestCase name,using this "this.getClass().getSimpleName()" to get the name of the class as the name of the class is the TestCase name.
			{
				...
				APP_LOGS.debug("skipping MyAccount_TC4(Never Miss a Deal Check) as the runmode is NO");
				throw new SkipException("Skipping this MyAccount_TC4(Never Miss a Deal Check) as the runmode is NO for this testcase");
		...
		@Test
		public void NeverMissADealCheck() throws InterruptedException
		{
			...
			APP_LOGS.debug("********* Starting the MyAccount_TC4(Never Miss a Deal Check) Link for guest user(No logged in user) ******* ");
			...
			APP_LOGS.debug(" Clicking on the Never Miss A Deal Link ");
			driver.findElement(By.linkText(OR.getProperty("NEVER_MISS_DEAL_LINK"))).click();
			wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.id(OR.getProperty("NEVER_MISS_DEAL_EMAIL_TEXT_ID"))));
			//Assert.assertTrue(driver.getPageSource().contains("You can track your individual order with your Order number"));
			APP_LOGS.debug("Entering emil ID to Subscribe");
			
		    driver.findElement(By.id(OR.getProperty("NEVER_MISS_DEAL_EMAIL_TEXT_ID"))).sendKeys(CONFIG.getProperty("EMAIL_NEVER_MISS_DEAL"));
		    driver.findElement(By.id(OR.getProperty("NEVER_MISS_DEAL_SUBSCRIBENOW_BUTTON_ID"))).click();
			
		    APP_LOGS.debug("Clicked on Subscribe NOW Button");
			 Thread.sleep(3000);
			...
			wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.xpath(OR.getProperty("NEVER_MISS_DEAL_POP_UP_VERIFICATION_XPATH"))));
			//System.out.println(driver.findElement(By.xpath(OR.getProperty("MY_ACCOUNT_VERIFICATION_TEXT_XPATH"))).getText());
			Assert.assertTrue(driver.getPageSource().contains("This e-mail is already subscribed at FutureBazaar"));
			driver.findElement(By.cssSelector("div.popup_close.f11")).click();	
			
			APP_LOGS.debug("Completed the Test case MyAccount_TC4(Never Miss a Deal Check)");
			
			//***********************************************
			}catch(Throwable t)
			{
				APP_LOGS.debug("******Execution of the MyAccount_TC4(Never Miss a Deal Check) is falied******");
