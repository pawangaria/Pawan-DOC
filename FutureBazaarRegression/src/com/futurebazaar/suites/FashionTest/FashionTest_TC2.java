...
public class FashionTest_TC2 extends FashionSuiteBase{
	...
	String runmodes[]=null;
		...
		public void checkTestSkip()
		{
				...
				//Instead of passing TestCase name,using this "this.getClass().getSimpleName()" to get the name of the class as the name of the class is the TestCase name.
			{
				...
				APP_LOGS.debug("skipping FashionTest_TC2(Fashion Link Inner_DD) as the runmode is NO");
				throw new SkipException("Skipping FashionTest_TC2 (FashionLinkInner_DD )testcase as the runmode is NO for this testcase");
		...
		@Test
		public void FashionLinkInner_DD() throws InterruptedException
		{
			//This method is used to check all the links in the fashion menu.
				...
				//****** Starting the Test for Fashion_TC2(Fashion Inner Check *******
			
				     APP_LOGS.debug("****** Starting the Test for Fashion_TC2(Fashion Inner Check ******* ");	
			         String  Fashion_InnerLink_Xpath =null;
					 WebElement kk = driver.findElement(By.xpath(OR.getProperty("FASHION_DROPDOWN_BLOCK_XPATH")));
					...
					//Getting the link count of Home &Living Drop down
					 APP_LOGS.debug("Getting the link count of Fashion Drop down");
					 int size1=kk4.size();
					 
					 for(int i=1;i<=size1;i++)
					 {
				    Fashion_InnerLink_Xpath="//div[4]/div/ul/li[2]/div/div/ul/li["+i+"]/a";
			    	...
			    	APP_LOGS.debug("***Completed the FashionTest_TC2 (Fashion outer DD  Check) ******");
			         }
				}
				catch(Throwable t)
			   {
				APP_LOGS.debug("******Execution of the Test case 2 (Fashion inner DD  Check) is falied******");
