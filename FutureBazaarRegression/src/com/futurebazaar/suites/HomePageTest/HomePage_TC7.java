...
public class HomePage_TC7 extends HomePageSuitebase{
	...
	String runmodes[]=null;
		...
		public void checkTestSkip()
		{
			...
			{   
				...
				APP_LOGS.debug("skipping test HomePage_TC7(Carousel Images Link Check By Clicking on Images) as the runmode is NO");
				throw new SkipException("Skipping this testcase as the runmode is NO for this testcase : HomePage_TC7(Carousel Images Link Check By Clicking on Images)");
		...
		@Test(dataProvider="getTestData")
		public void CarouselImageLinkCheckByClicking() throws InterruptedException
		{
			try{
			count++;
			...
			//At last verify the links by clicking the link and then click all the Image in Carousel.
			...
			//getting the images count to loop the images.
			...
			APP_LOGS.debug("No of Carousel Images on hame page are "+image.size());
			
			APP_LOGS.debug("verifying the CAROUSEL Image as a Link by clicking on images one by one");
			//for loop is applied to click on every image which is as a link.
			
			for(int i=1;i<=imagecount_image;i++)
			{
			...
			driver.findElement(By.xpath(OR.getProperty("CAROUSEL_IMAGE_CLICK_XPATH"))).click();
			...
			APP_LOGS.debug("*******Execution for the HomePage_TC7(Carousel Images Link Check By Clicking on Images) is verified");
			
				//***********************************************
			}catch(Throwable t)
			{
				APP_LOGS.debug("*******Execution for the HomePage_TC7(CarouselLink Check By Clicking on home page) is falied*****");
